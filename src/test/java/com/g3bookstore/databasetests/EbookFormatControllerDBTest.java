...
/**
 * Test class for the Ebook format controller
 *
 * @author Kevin
 */
@Ignore
@RunWith(Arquillian.class)
public class EbookFormatControllerDBTest {

    @Inject
    private CustomEbookFormatJpaController ebookController;

    @Resource(name = "java:app/jdbc/bookstore")
    private DataSource ds;

    private Logger log = Logger.getLogger(EbookFormatControllerDBTest.class.getName());

    public EbookFormatControllerDBTest() {
    }
    ...
    /**
     * Test that checks if all the books for the specified format is returned
     * correctly.
     * 
     * @throws SQLException 
     */
    @Test
    public void findAllBooksPerFormat() throws SQLException {
        EbookFormat ebf = ebookController.getEbookFormatByID(1);
        List<Book> books = ebookController.getAllBooksByFormat(ebf);
        log.log(Level.INFO, "Data>>>{0}", books.get(0).getFormatId());
        assertThat(books).hasSize(20);
    }
    
    /**
     * Test which will check if the correct ebook format is returned when searching 
     * by primary key.
     * 
     * @throws SQLException
     */
    @Test
    public void findEbookFormatByPrimaryKeyTest() throws SQLException
    {
        EbookFormat ebf = ebookController.getEbookFormatByID(1);
        log.log(Level.INFO, "Data>>>{0}", ebf.getBookFormat());
        assertEquals(ebf.getBookFormat(), "AZW");
    }
